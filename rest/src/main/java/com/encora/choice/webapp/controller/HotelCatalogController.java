...
public class HotelCatalogController {
    ...
    @GetMapping
    public SearchPage searchPage(@RequestParam @Min(1) Integer page, @RequestParam @Min(1) Integer size, @RequestParam @Size(min = 3) String searchTerm) {
        log.info("Performing search request for page:" + page + " size:" + size + " and term:" + searchTerm);
        return service.searchHotel(Optional.of(page).orElse(1), Optional.of(size).orElse(defaultPageSize), searchTerm);
    }
